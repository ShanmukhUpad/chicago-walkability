...
cor_data <- city_data %>%
  ...
  select(walk_score, intersection_density, employment_entropy,
         transit_distance_km, workers_per_hh) %>%
  na.omit()
...
maps and by the Moran's I test, which shows significant positive spatial
autocorrelation. Within the City of Chicago itself, however, no single
predictor is a *strong* bivariate driver of walkability. Economic
opportunity is the only variable that reaches a moderate correlation
(r = 0.369, R² = 0.136); intersection density (r = 0.231), transit
distance (r = −0.238), employment entropy (r = 0.283), and workers per
household (r = 0.224) are all in the weak range. Population density
within the city is essentially uncorrelated with walkability
(r = −0.049), so the *mechanistic* part of the hypothesis — that high
density alone produces high walkability — is not supported at the
bivariate level inside Chicago. Population density was therefore
dropped from the chart panels. The story the data does support is the
*composite* one: the multivariate regression of transit distance,
intersection density, employment entropy, and population density on
walkability explains R² ≈ 0.40 of tract-level variance within the city,
which is consistent with the hypothesis that the urban form variables
together drive higher walkability in central Chicago.
...
opportunity is the strongest *bivariate* predictor of walkability
within the City of Chicago (r ≈ 0.37, R² ≈ 0.14) — moderate but not
strong. Tracts with higher opportunity tend to have higher walkability
scores, but a large amount of tract-level variation remains
unexplained, which is why the multivariate regression is the more
useful summary of the urban-form drivers.
...
**Workers per Household Visualization:**

This map shows the average number of workers per household in each
Chicago census tract. It is included as a labor-market correlate of
the urban-form bundle that the hypothesis points to. Within the city,
the bivariate correlation with walkability is r ≈ 0.22 — weak but
statistically significant — so the map is descriptive context rather
than a strong predictor. A jenks classification is used.
...
walkability is r ≈ −0.24 (weak, negative): tracts farther from transit
tend to score slightly lower. In the multivariate regression, however,
transit distance has the largest |t| of any single predictor inside
the city, so it pulls a lot of weight once collinearity with the
other urban-form variables is accounted for.
...
reflect weak-to-moderate associations.
...
```{r additional-visualizations, fig.width=12, fig.height=12}
...
# Maps restricted to variables with at least a weak bivariate correlation
# with walkability inside the City of Chicago (|r| >= 0.20). Population
# density is excluded because its city-extent r is -0.05 (negligible).

# Visualization 1: Street Intersection Density (city r = 0.231)
...
# Visualization 2: Economic Opportunity (city r = 0.369, moderate)
...
# Visualization 3: Workers per Household (city r = 0.224)
...
# Visualization 4: Transit Distance (city r = -0.238)
...
tmap_arrange(tm2, tm3, tm4, tm5, ncol = 2)
```
...
tibble(
  Variable  = c("Street Intersection Density",
                "Employment Entropy (Mixed Use)",
                "Transit Distance (km)",
                "Workers per Household",
                "Economic Opportunity"),
  r         = round(c(ct_int$estimate, ct_emp$estimate, ct_trn$estimate,
                      ct_wkr$estimate, ct_opp$estimate), 3),
  `R²`      = round(r^2, 3),
  `p-value` = c(ct_int$p.value, ct_emp$p.value, ct_trn$p.value,
                ct_wkr$p.value, ct_opp$p.value),
  Sig       = dplyr::case_when(
    `p-value` < 0.001 ~ "***",
    `p-value` < 0.01  ~ "**",
    `p-value` < 0.05  ~ "*",
  ...
  knitr::kable(caption = "Bivariate Pearson correlations with walkability score within the City of Chicago. Population density (r = -0.05) excluded as negligible.")
```
...
ggplot(scatter_data, aes(x = opportunity, y = walk_score)) +
  ...
  labs(
    ...
    subtitle = paste0("r = ", r_opp, ", R² = ", r2_opp,
                      " — strongest bivariate predictor of walkability within the City of Chicago"),
...
pairs_data <- city_data %>%
  ...
  select(walk_score, transit_distance_km,
         intersection_density, employment_entropy, workers_per_hh) %>%
  na.omit()
...
CBD/Loop and falling off in outer neighborhoods. Two findings support
the hypothesis. First, the Moran's I test confirms that walkability is
not randomly distributed but follows clear neighborhood-scale spatial
patterns, and the maps show walkability is highest in and around the
Loop, exactly as predicted. Second, the multivariate regression of
transit distance, intersection density, employment entropy, and
population density on walkability within the city explains R² ≈ 0.40
of tract-level variance — a strong joint effect that is consistent
with walkability being driven by an urban-form bundle rather than any
single variable.

The mechanistic part of the hypothesis — that high population density
*by itself* drives higher walkability — is **not** supported at the
bivariate level once the analysis is restricted to Chicago city tracts:
Pearson r between population density and walkability is −0.049, which
is negligible. The most likely reason is range compression. Across the
city, the densest residential tracts (Lakeview, Edgewater, parts of
Lincoln Park) are not the most walkable tracts; the Loop has the
highest walkability but a comparatively low residential density because
it is dominated by jobs and retail. Pop density therefore loses
explanatory power when it is the *only* predictor in view inside the
city, even though it remains statistically significant in the
multivariate regression after controlling for the other urban-form
variables.
...
town or city can be accessed within the span of a 15-minute walk. Even
in some low-density suburbs, there is demand for a reduction in
car-centric infrastructure. The bivariate correlations within Chicago
all sit in the weak-to-moderate band: economic opportunity (r = 0.369,
moderate), employment entropy (r = 0.283), transit distance
(r = −0.238), intersection density (r = 0.231), and workers per
household (r = 0.224). Each is statistically significant — the
n = 866 tract sample makes even small correlations distinguishable
from zero — but no single variable is doing the heavy lifting on its
own. The multivariate regression's R² ≈ 0.40 is the better summary:
walkability is the joint product of mixed-use development, dense
intersections, and transit proximity, which is what the "15-minute
city" aims for. Tracts can therefore be relatively low-density and
still score high on walkability if those other urban-form attributes
are present.
