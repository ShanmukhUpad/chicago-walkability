...
```{r lisa, fig.width=10, fig.height=8}
# Local Indicators of Spatial Association. Only run if the global Moran's I
# detected significant clustering, since LISA quadrants are uninterpretable
# without underlying spatial autocorrelation.
if (moran_test$p.value < 0.05) {
  ...
  cat("LISA cluster counts (p < 0.05):\n")
...
Chicago, the 8-tier employment mix (D2B_E8MIXA) correlates strongly
with walkability (r ≈ 0.68), employment + household entropy
(D2A_EPHHM) is also strong (r ≈ 0.60), intersection density (D3B) is
moderate (r ≈ 0.42), and transit distance (D4A) is moderate negative
(r ≈ −0.30). These are the underlying components of the EPA formula,
...
one of the strongest bivariate predictors of walkability (r ≈ 0.60,
R² ≈ 0.36): tracts with greater mixed-use intensity score
...
walkability is r ≈ −0.30 (moderate, negative): tracts farther from
transit tend to score lower. Transit distance is one of the four
direct inputs to the EPA walkability formula, so the negative
relationship is by construction.
...
# Visualization 1: Street Intersection Density (city r ≈ 0.42, moderate)
...
# Visualization 3: Transit Distance (city r ≈ -0.30, moderate negative)
...
strong: 8-tier employment mix (D2B_E8MIXA) reaches r ≈ 0.68,
employment + household entropy (D2A_EPHHM) reaches r ≈ 0.60,
intersection density (D3B) reaches r ≈ 0.42, and transit distance
(D4A) reaches r ≈ −0.30. These are the underlying components of the
...
regression on Chicago city tracts explains R² ≈ 0.84 of walkability
